(async function SacPrevencaoV6() {
  "use strict";

  const APP = "sac_prevencao_v6_20260620";
  const BUILD = "ANALISE/V6";
  ...
  const CLIPBOARD_ENVELOPE_TYPE = "web application/x-sac-prevencao-v6";
  const MEMORY_BOOTSTRAP_KEY = "__SAC_PREVENCAO_SHARED_MEMORY";
  ...
  if (storageGet("activeBuild") !== BUILD_VERSION) {
    ["lastFalcon", "lastConsole", "history", "executionLists"].forEach(storageRemove);
    storageSet("activeBuild", BUILD_VERSION);
  }
  function clearPreviousRuntime() {
    all("#sac-style,#sac-style-v6,.sac-panel,.sac-history-panel,.sac-choice-popover,#sac-notices").forEach((node) => node.remove());
  ...
  let clipboardEnvelopeReady = false;
  function normalizeSharedMemory(value) {
    const memory = value && typeof value === "object" ? value : {};
    return {
      buildVersion: BUILD_VERSION,
      savedAt: Number(memory.savedAt || 0),
      lists: Array.isArray(memory.lists) ? memory.lists : [],
      history: Array.isArray(memory.history) ? memory.history : []
    };
  }
  let sharedMemoryCache = normalizeSharedMemory(window[MEMORY_BOOTSTRAP_KEY]);
  function setSharedMemory(memory) {
    sharedMemoryCache = normalizeSharedMemory(memory);
    sharedMemoryCache.savedAt = Date.now();
    window[MEMORY_BOOTSTRAP_KEY] = sharedMemoryCache;
    return sharedMemoryCache;
  }
  async function readClipboardEnvelope() {
    if (sharedMemoryCache.savedAt || sharedMemoryCache.lists.length || sharedMemoryCache.history.length) {
      return sharedMemoryCache;
    }
    ...
    try {
      ...
      for (const item of items) {
        if (!item.types.includes(CLIPBOARD_ENVELOPE_TYPE)) continue;
        const blob = await item.getType(CLIPBOARD_ENVELOPE_TYPE);
        const envelope = JSON.parse(await blob.text());
        if (envelope?.buildVersion === BUILD_VERSION && Array.isArray(envelope.lists)) {
          return setSharedMemory(envelope);
        }
      }
    } catch (_err) {}
    return null;
  }
  async function writeClipboardPayload(text, memory = sharedMemoryCache) {
    const nextMemory = setSharedMemory(memory);
    if (navigator.clipboard?.write && window.ClipboardItem) {
      try {
        const envelope = JSON.stringify({
          buildVersion: BUILD_VERSION,
          savedAt: Date.now(),
          lists: nextMemory.lists,
          history: nextMemory.history
        });
        await navigator.clipboard.write([new ClipboardItem({
          "text/plain": new Blob([String(text ?? "")], { type: "text/plain" }),
          [CLIPBOARD_ENVELOPE_TYPE]: new Blob([envelope], { type: CLIPBOARD_ENVELOPE_TYPE })
        })]);
        clipboardEnvelopeReady = true;
        return true;
      } catch (_err) {}
    }
    clipboardEnvelopeReady = false;
    try {
  ...
  async function copyText(text, lists = null, history = null) {
    const current = await readClipboardEnvelope() || sharedMemoryCache;
    const memory = {
      buildVersion: BUILD_VERSION,
      lists: Array.isArray(lists) ? lists : (current.lists?.length ? current.lists : readExecutionListFallback()),
      history: Array.isArray(history) ? history : (current.history?.length ? current.history : readHistoryLocal())
    };
    if (await writeClipboardPayload(text, memory)) return true;
    try {
  ...
  function openChoicePopover({ id, title, options, selected = [], extraInput = null, onSave }) {
    ...
    panel.innerHTML = `
      <div class="sac-choice-head">
        ...
        <button data-close aria-label="Fechar">×</button>
      </div>
    ...
    if (host) {
      ...
      panel.style.left = `${Math.max(8, rect.left - 362)}px`;
    ...
    all("[data-close]", panel).forEach((button) => button.addEventListener("click", () => panel.remove()));
  ...
  function placePidPanel() {
    ...
    const rect = host.getBoundingClientRect();
    panel.style.left = `${Math.max(8, rect.left - 366)}px`;
    panel.style.right = "auto";
    panel.style.top = `${Math.max(8, rect.top)}px`;
  }
  ...
  function openPidPanel(data) {
    ...
    placePidPanel();
    panel.querySelector("[data-close]")?.addEventListener("click", closePidPanel);
  }

  function ensureStyles() {
    if (byId("sac-style-v6")) return;
    const style = document.createElement("style");
    style.id = "sac-style-v6";
    style.textContent = `
      .sac-panel{--sac-font-scale:1;--sac-primary:#00856f;--sac-panel-width:420px;position:fixed;top:8px;right:8px;z-index:2147483647;box-sizing:border-box!important;inline-size:var(--sac-panel-width)!important;width:var(--sac-panel-width)!important;min-inline-size:var(--sac-panel-width)!important;min-width:var(--sac-panel-width)!important;max-inline-size:var(--sac-panel-width)!important;max-width:var(--sac-panel-width)!important;border:1px solid var(--sac-border);border-top:3px solid var(--sac-primary);border-radius:8px;background:var(--sac-bg);color:var(--sac-text);font-family:Inter,Segoe UI,Arial,sans-serif;box-shadow:0 18px 44px rgba(0,0,0,.30);overflow:visible;text-align:left}
      ...
      .sac-choice-popover{position:fixed;right:14px;top:72px;z-index:2147483647;width:min(354px,calc(100vw - 28px));display:grid;gap:6px;padding:0 8px 8px;border:1px solid var(--sac-border);border-top:3px solid var(--sac-primary);border-radius:8px;background:var(--sac-bg);color:var(--sac-text);box-shadow:0 18px 44px rgba(0,0,0,.32);font-family:Inter,Segoe UI,Arial,sans-serif;box-sizing:border-box!important}.sac-choice-popover *{box-sizing:border-box!important}.sac-choice-head{margin:0 -8px;padding:7px 8px;background:var(--sac-primary);color:#fff;display:flex;justify-content:space-between;align-items:center;border-radius:6px 6px 0 0}.sac-choice-head strong{font-size:12px}.sac-choice-head button{width:24px;height:24px;padding:0;border-color:rgba(255,255,255,.4);background:rgba(127,29,29,.32)}.sac-choice-grid{display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr);gap:4px}.sac-choice-popover label{display:flex;gap:5px;align-items:flex-start;min-width:0;min-height:34px;border:1px solid var(--sac-border);border-radius:5px;background:var(--sac-card);padding:5px;font-size:9.5px;font-weight:850;line-height:1.15;cursor:pointer;word-break:normal;overflow-wrap:normal;hyphens:none;white-space:normal}.sac-choice-popover label>span{display:block;min-width:0;white-space:normal;word-break:normal;overflow-wrap:normal;hyphens:none}.sac-choice-popover label:hover{border-color:#38bdf8;background:#10263a;color:#edf3fb}.sac-light.sac-choice-popover label:hover{background:#eef7ff;color:#172033}.sac-choice-popover input[type="checkbox"]{flex:0 0 auto;margin:1px 0 0}.sac-choice-extra{display:grid!important;gap:4px;min-height:0!important;font-size:10.5px!important}.sac-choice-extra input{width:100%;height:30px;border:1px solid var(--sac-border);border-radius:6px;background:var(--sac-input);color:var(--sac-text);padding:5px;font-weight:850}.sac-choice-popover .sac-choice-actions{display:grid;grid-template-columns:1fr 1fr;gap:6px}.sac-choice-popover button{border:1px solid var(--sac-border);border-radius:6px;background:var(--sac-primary);color:#fff;padding:8px;font-weight:950;cursor:pointer}.sac-choice-popover button.secondary{background:transparent;color:var(--sac-text)}.sac-pid-panel{width:360px!important;min-width:360px!important;max-width:360px!important;padding:0 10px 10px!important}.sac-pid-panel .sac-choice-head{margin:0 -10px}.sac-pid-groups{display:grid;gap:7px}.sac-pid-group{display:grid;gap:4px}.sac-pid-group strong{font-size:10px;color:var(--sac-muted);text-transform:uppercase}.sac-pid-grid{display:grid;grid-template-columns:1fr;gap:4px}.sac-pid-card{border:1px solid var(--sac-border);border-radius:6px;background:var(--sac-card);padding:5px 7px;min-height:31px;font-size:10.5px;font-weight:850;line-height:1.12;display:grid;grid-template-columns:22px 1fr;align-items:center;gap:5px}.sac-pid-card b{display:grid;place-items:center;width:20px;height:20px;border-radius:4px;background:var(--sac-primary);color:#fff;font-size:9px}.sac-pid-note{border:1px solid #f59e0b;border-radius:6px;background:#3a230b;color:#fff7ed;padding:6px;font-size:10.5px;font-weight:900;line-height:1.18}.sac-light .sac-pid-note{background:#fff7ed;color:#7c2d12}
  ...
  function renderPanel({ id, stage, flow = "banking", subtitle = "", body = "", footer = "", onEnter, onSignature }) {
    ...
    const close = () => {
      ...
      if (panel.dataset.finalLocked === "true") {
        showNotice("Para finalizar esta etapa, clique em Copiar. Para rever a análise, use Mudar decisão.", "info", 10000);
        return false;
      }
    ...
    panel.querySelector("[data-action='save-signature']")?.addEventListener("click", () => {
      ...
      showNotice("Assinatura salva.", "success");
      onSignature?.();
    });
  ...
  window.__SAC_PREVENCAO_V6_KEYS = (event) => {
  ...
  document.addEventListener("keydown", window.__SAC_PREVENCAO_V6_KEYS);
  ...
  function kvOptional(label, value, cls = "") {
    return `<div class="sac-kv ${cls}"><div class="sac-kv-label">${escapeHtml(label)}</div><div class="sac-kv-value">${escapeHtml(clean(value, ""))}</div></div>`;
  }
  ...
  async function renderConsole() {
    ...
    const syncCardCall = () => {
      const enabled = normalize(document.querySelector("input[name='sac-call-mode']:checked")?.value) === "COM CHAMADA";
      const box = panel.querySelector("[data-call-result-box]");
      if (box) box.setAttribute("aria-disabled", enabled ? "false" : "true");
      all("input[name='sac-call-result']", panel).forEach((input) => {
        input.disabled = !enabled;
        if (!enabled) input.checked = false;
      });
      data.fields.callMode = enabled ? "com chamada" : "sem chamada";
      data.fields.callResult = enabled ? (document.querySelector("input[name='sac-call-result']:checked")?.value || "") : "";
      if (data.flow === "card" && enabled) openPidPanel(data);
      else closePidPanel();
    };
    all("input[name='sac-call-mode'],input[name='sac-call-result']", panel).forEach((input) => input.addEventListener("change", syncCardCall));
    syncCardCall();
    enableManualGridEditing(panel, data);
  ...
  function cardFields(data) {
    ...
    const resultDisabled = mode !== "com chamada";
    const call = `
      <div class="sac-call-grid">
        <div class="sac-call-box">
          <strong>Chamada</strong>
          <label><input type="radio" name="sac-call-mode" value="sem chamada" ${mode === "sem chamada" ? "checked" : ""}>Sem chamada</label>
          <label><input type="radio" name="sac-call-mode" value="com chamada" ${mode === "com chamada" ? "checked" : ""}>Com chamada</label>
        </div>
        <div class="sac-call-box" data-call-result-box aria-disabled="${resultDisabled ? "true" : "false"}">
          <strong>Resultado</strong>
          <label><input type="radio" name="sac-call-result" value="com sucesso" ${result === "com sucesso" ? "checked" : ""} ${resultDisabled ? "disabled" : ""}>Com sucesso</label>
          <label><input type="radio" name="sac-call-result" value="sem sucesso" ${result === "sem sucesso" ? "checked" : ""} ${resultDisabled ? "disabled" : ""}>Sem sucesso</label>
        </div>
      </div>`;
    return call
      + field("sac-merchant-history", "Histórico no estabelecimento", CARD_REVIEW, data.fields.merchantHistory, { allowEmpty: true })
  ...
  async function readConsoleFields(data) {
    if (data.flow === "card") {
      return {
        callMode: document.querySelector("input[name='sac-call-mode']:checked")?.value || "sem chamada",
        callResult: document.querySelector("input[name='sac-call-result']:checked")?.value || "",
  ...
  async function renderTabulator(existingData = null) {
    ...
    data.__primaryApplication = applyPrimaryTabulatorFields(data);
    const missing = requiredWorkflow(data);
  ...
  function consoleDropdownGrid(data) {
    if (data.flow === "card") {
      const callResult = normalize(data.fields.callMode) === "COM CHAMADA" ? kv("Resultado da chamada", data.fields.callResult, dropdownAlert(data.fields.callResult, ["com sucesso"])) : "";
      return `<div class="sac-grid">${kv("Chamada", data.fields.callMode || "sem chamada")}${callResult}${kv("Histórico no estabelecimento", data.fields.merchantHistory, cardReviewAlert(data.fields.merchantHistory))}${kv("Padrão de compra", data.fields.purchasePattern, cardReviewAlert(data.fields.purchasePattern))}</div>`;
    }
    ...
    return `<div class="sac-grid three">${personStatusGrid}${spdHistoryGrid}${kv("Mídia desabonadora", data.fields.badMedia, dropdownAlert(data.fields.badMedia, MEDIA_OPTIONS))}${kv("E-mail, DDD e Endereço", data.fields.emailPhoneAddress, dropdownAlert(data.fields.emailPhoneAddress, EMAIL_OPTIONS))}${kv("Documentação", data.fields.documentation, dropdownAlert(data.fields.documentation, DOC_OPTIONS))}${kv("Extrato", data.fields.statement, dropdownAlert(data.fields.statement, STATEMENT_OPTIONS))}</div>`;
  }

  let activeTabulatorDecisionRun = 0;
  function setDecisionApplicationState(panel, message, type = "", applying = true) {
    panel.dataset.decisionApplying = applying ? "true" : "false";
    let status = panel.querySelector("#sac-decision-application-status");
    if (!status) {
      status = document.createElement("div");
      status.id = "sac-decision-application-status";
  ...
  async function applyDecisionAndShowFinal(data, decision, panel) {
    ...
    setDecisionApplicationState(panel, "Aplicando os campos no Tabulador. Aguarde a confirmação.", "", true);
    showNotice("Por favor, aguarde um instante: estou aplicando a decisão e preparando a tabulação.", "info", 9000);
    fillAnyImmediate(observationTargets, text);
    await wait(0);
    const missing = requiredWorkflow(data);
    if (missing.length) {
      if (getSafeMode()) {
        ...
        setDecisionApplicationState(panel, `Inconsistência encontrada em: ${location}.`, "error", false);
    ...
    const applied = await applyTabulator(data, decision, text, isCurrentRun);
    if (applied.cancelled || !isCurrentRun()) return;
    ...
    if (criticalPending.length) {
      ...
      setDecisionApplicationState(panel, `Inconsistência obrigatória em: ${location}.`, "error", false);
      setDecisionButtonsEnabled(panel, true);
      return;
    }
    if (!applied.ok) {
      if (getSafeMode()) {
        ...
        setDecisionApplicationState(panel, `Inconsistência encontrada em: ${location}.`, "warn", false);
        setDecisionButtonsEnabled(panel, true);
        return;
      }
      showNotice(`Ação necessária: confira ${tabulatorIssueMessage(applied.pending)}.`, "warn-pulse", 15000);
    }
    setDecisionApplicationState(panel, "Campos aplicados. Abrindo a tabulação pronta.", applied.ok ? "ok" : "warn", false);
    showFinalTabulation(
      data,
      decision,
      text,
  ...
  function showFinalTabulation(data, decision, text, panel, status = "") {
    ...
    byId("sac-copy-final")?.addEventListener("click", async (event) => {
      ...
      const queue = await updateListsForFinalDecision(data, decision);
      writeExecutionListFallback(queue);
      ...
      if ((queue.length || history.length) && !clipboardEnvelopeReady) {
        showNotice("A tabulação foi copiada, mas o Chrome não preservou a memória de LISTAS e Histórico. Mantenha esta página aberta e tente novamente.", "error", 14000);
  ...
  function fillAnyImmediate(targets, value) {
    ...
    targets.forEach((target) => {
      ...
      if (element) ok = setNativeValue(element, value) || ok;
    });
    return ok;
  }
  async function forceFillAny(targets, value, tries = 7, delay = 40) {
    if (isMissing(value)) return false;
  ...
  async function waitForTabulatorFields() {
    return waitForField(() => byId("txt_ValorTransacao") || byId("ddl_status") || elementByName("_partial_Falcon.NumeroCaso"), 35, 55);
  }
  ...
  async function selectDropdown(id, wanted, tries = 15, isActive = () => true) {
  ...
  async function selectDependentDropdown(parentId, parentWanted, childId, childWanted, tries = 60, isActive = () => true) {
    if (isMissing(childWanted)) return true;
    if (!await selectDropdown(parentId, parentWanted, 35, isActive)) return false;
    for (let i = 0; i < tries; i += 1) {
      if (!isActive()) return false;
      if (!dropdownSelectionMatches(parentId, parentWanted)) {
        if (!await selectDropdown(parentId, parentWanted, 4, isActive)) {
          await wait(80);
          continue;
        }
      }
      if (await selectDropdown(childId, childWanted, 1, isActive)) {
        await wait(220);
        if (!isActive()) return false;
        if (dropdownSelectionMatches(parentId, parentWanted) && dropdownSelectionMatches(childId, childWanted)) return true;
      }
      await wait(80);
    }
    return false;
  }
  async function applyDecisionDropdowns(data, decision, isActive = () => true) {
    const reason = reasonForDecision(data.flow, decision);
    const reasonOk = reason
      ? await selectDependentDropdown("ddl_status", decision, "ddl_motivostatus", reason, 75, isActive)
      : await selectDropdown("ddl_status", decision, 40, isActive);
    if (!isActive()) return { statusOk: false, reasonOk: false, cancelled: true };
    if (reason && !dropdownSelectionMatches("ddl_motivostatus", reason)) {
      await selectDependentDropdown("ddl_status", decision, "ddl_motivostatus", reason, 30, isActive);
    }
    return {
      statusOk: dropdownSelectionMatches("ddl_status", decision),
      reasonOk: !reason || dropdownSelectionMatches("ddl_motivostatus", reason)
    };
  }
  ...
  function tabulatorCallValues(data) {
    if (data.flow === "card" && normalize(data.fields?.callMode) === "COM CHAMADA") {
      return {
        type: "ATIVA - PLANILHA",
  ...
  async function applyPrimaryTabulatorFields(data) {
    const pending = [];
    const f = data.falcon || {};
    ...
    const inputTasks = [
      forceFillAny([{ id: "txt_ValorTransacao" }, { name: "_partial_Falcon.ValorTransacao" }, { pattern: /valor.*transa/i }], clean(f.value, "").replace("R$", "").trim(), 3, 25).then((ok) => { if (!ok) addPending("Valor da transação"); }),
      forceFillAny([{ name: "_partial_Falcon.NumeroCaso" }, { id: "txt_NumeroCaso" }, { pattern: /numero.*caso|caso.*numero/i }], f.caseNumber, 3, 25).then((ok) => { if (!ok) addPending("Número do caso"); }),
      forceFillAny([{ name: "_partial_Falcon.EcTransacao" }, { pattern: /ecTransacao|estabelecimento|tipoTransacao/i }], data.flow === "card" ? f.merchant : f.transactionType, 3, 25).then((ok) => { if (!ok) addPending(data.flow === "card" ? "Estabelecimento" : "Tipo de transação"); }),
      forceFillAny([{ name: "_partial_Falcon.RegraListada" }, { pattern: /regra.*list/i }], f.rule, 3, 25).then((ok) => { if (!ok) addPending("Regra"); })
    ];
    if (f.transactionDate?.includes("/")) {
      const [date, time] = f.transactionDate.split(/\s+/);
      if (date) inputTasks.push(forceFillAny([{ id: "txt_data_entrada" }, { name: "_partial_Falcon.DataEntrada" }, { pattern: /data.*entrada/i }], date.split("/").reverse().join("-"), 3, 25).then((ok) => { if (!ok) addPending("Data"); }));
      if (time) inputTasks.push(forceFillAny([{ id: "txt_hora_entrada" }, { name: "_partial_Falcon.HoraEntrada" }, { pattern: /hora.*entrada/i }], trimTimeToMinute(time), 3, 25).then((ok) => { if (!ok) addPending("Hora"); }));
    }

    const documentTask = (async () => {
      ...
      const typeApplied = await selectDropdown("ddl_tipoDoc", docKind, 18)
        || await selectDropdownByPattern(/tipo.*doc|document.*type/i, docKind, 14);
      ...
      if (!await forceFillAny(targets, doc, 5, 35)) addPending(docKind);
    })();
    const callValues = tabulatorCallValues(data);
    const queue = queueFor(data);
    const selectTasks = [
      byId("ddl_tabulador") ? selectDropdown("ddl_tabulador", "Falcon", 12).then((ok) => { if (!ok) addPending("Tabulador Falcon"); }) : Promise.resolve(),
      selectIssuerDropdown(data.issuer).then((ok) => { if (!ok) addPending("Emissor"); }),
      selectDropdown("ddl_TipoChamada", callValues.type, 16).then((ok) => { if (!ok) addPending("Tipo de chamada"); }),
      selectDropdown("ddl_ChamadaAtiva", callValues.result, 16).then((ok) => { if (!ok) addPending("Status chamada"); }),
      queue ? (async () => {
        let ok = await selectDropdown("ddl_Fila", queue, 60);
        if (!ok) {
          await wait(180);
          ok = await selectDropdown("ddl_Fila", queue, 35);
        }
        if (!ok || !dropdownSelectionMatches("ddl_Fila", queue)) addPending("Fila");
      })() : Promise.resolve()
    ];
    await Promise.allSettled([...inputTasks, documentTask, ...selectTasks]);
    primeTabulatorFields(data, "", "");
    data.__primaryPending = pending;
    return { ok: pending.length === 0, pending };
  }
  async function applyTabulator(data, decision, tabulationText = "", isActive = () => true) {
    const primary = await (data.__primaryApplication || applyPrimaryTabulatorFields(data));
    ...
    const decisionFields = await applyDecisionDropdowns(data, decision, isActive);
    if (decisionFields.cancelled || !isActive()) return { ok: false, pending: [], cancelled: true };
    if (!decisionFields.statusOk) addPending("Decisão"); else removePending("Decisão");
    if (!decisionFields.reasonOk) addPending("Motivo status"); else removePending("Motivo status");
    primeTabulatorFields(data, decision, tabulationText);
    const observationTargets = [{ id: "txt_obs" }, { name: "_partial_Falcon.Observacao" }, { pattern: /observa|descricao|comentario/i, selector: "textarea,input" }];
    if (!await forceFillAny(observationTargets, tabulationText, 5, 35)) addPending("Observações");
    return { ok: pending.length === 0, pending };
  }
  ...
  let executionListFallback = [];
  function readExecutionListFallback() { return executionListFallback; }
  function writeExecutionListFallback(list) { executionListFallback = Array.isArray(list) ? list : []; }
  async function readListQueue() {
    const remote = (await readClipboardEnvelope())?.lists || sharedMemoryCache.lists || [];
    const local = readExecutionListFallback();
    const byItemId = new Map();
    [...(Array.isArray(remote) ? remote : []), ...local].forEach((item) => {
    ...
    writeExecutionListFallback(filtered);
    sharedMemoryCache.lists = filtered;
    return filtered;
  }
  async function writeListQueue(list) {
    const next = list.slice(0, 300);
    writeExecutionListFallback(next);
    const text = await navigator.clipboard?.readText?.().catch(() => "") || "";
    return writeClipboardPayload(text, {
      ...sharedMemoryCache,
      lists: next,
      history: readHistory()
    });
  ...
  async function updateListsForFinalDecision(data, decision) {
    ...
    if (!lists.allowlist && !lists.contencao) return queue;
    ...
    return [{ ...item, issuerId }, ...withoutCurrentCase];
  }
  ...
  async function markListDone(id, listType) {
    const list = (await readListQueue()).map((item) => {
  ...
  async function removeListItem(id, listType) {
    await markListDone(id, listType);
  }
  async function renderLists(activeTab = storageGet("activeListTab") || "allowlist") {
    ...
    writeExecutionListFallback(items);
    sharedMemoryCache.lists = items;
    ...
    panel.addEventListener("click", async (event) => {
      ...
      if (event.target.closest("[data-list-remove]")) {
        await removeListItem(id, listType);
  ...
  function readHistory() {
    const now = Date.now();
    const byId = new Map();
    [...sharedMemoryCache.history, ...readHistoryLocal()].forEach((item) => {
      if (!item?.id || now - Number(item.savedAt || 0) >= HISTORY_TTL_MS) return;
      const cleanItem = { ...item, tabulation: redactSensitiveDocuments(item.tabulation || "") };
      const existing = byId.get(cleanItem.id);
      if (!existing || Number(cleanItem.savedAt || 0) >= Number(existing.savedAt || 0)) byId.set(cleanItem.id, cleanItem);
    });
    const history = Array.from(byId.values())
      .sort((a, b) => Number(b.savedAt || 0) - Number(a.savedAt || 0))
      .slice(0, 60);
    writeJson("history", history);
    sharedMemoryCache.history = history;
    return history;
  }
  function addHistory(data, decision, tabulation) {
    ...
    const history = [
      item,
      ...readHistory().filter((entry) => {
        const entryIdentity = `${alnumOnly(entry.caseNumber)}:${alnumOnly(entry.account)}`;
        return entry.id !== item.id && entryIdentity !== identity;
      })
    ].slice(0, 60);
    writeJson("history", history);
    sharedMemoryCache.history = history;
