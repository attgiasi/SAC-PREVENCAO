...
const bookmarkletLoaderRef = bookmarklet.match(/@([a-f0-9]{40})\/ANALISE\/V12\/loader-v12\.js\?v=12\.4\.0/)?.[1] || "";

assert.match(safeFallback, /^[a-f0-9]{40}$/);
assert.equal(release.build, "12.4");
...
(async () => {
  ...
  assert.ok(viaApi.loaded.every((url) => url.includes("v=12.4.0-")));
