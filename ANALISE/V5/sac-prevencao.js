(async function SacPrevencaoV5() {
  "use strict";

  const APP = "sac_prevencao_v5_20260619";
  const BUILD = "ANALISE/V5";
  ...
  const MERCHANT_HISTORY_OPTIONS = ["sim", "não", "sem histórico", "ausência de dados"];
  const PURCHASE_PATTERN_OPTIONS = ["dentro do padrão", "fora do padrão", "ausência de dados"];
  ...
  function pidProfileFor(data) {
    ...
    return {
      ...
      complementary: isAmigoz ? [] : PID_DEFAULT_COMPLEMENTARY,
      note: isAmigoz
  ...
  function openPidPanel(data) {
    ...
    const cards = (items) => items.map((item) => `<div class="sac-pid-card">${escapeHtml(item)}</div>`).join("");
    panel.innerHTML = `
      <div class="sac-choice-head">
    ...
    if (host) {
      const rect = host.getBoundingClientRect();
      panel.style.left = `${Math.max(8, rect.left - 374)}px`;
      panel.style.right = "auto";
      panel.style.top = `${Math.max(8, rect.top)}px`;
    }
  ...
  function ensureStyles() {
    ...
    style.id = "sac-style";
    style.textContent = `
      .sac-panel{--sac-font-scale:1;--sac-primary:#00856f;position:fixed;top:8px;right:8px;z-index:2147483647;width:420px;min-width:420px;max-width:420px;box-sizing:border-box;border:1px solid var(--sac-border);border-top:3px solid var(--sac-primary);border-radius:8px;background:var(--sac-bg);color:var(--sac-text);font-family:Inter,Segoe UI,Arial,sans-serif;box-shadow:0 18px 44px rgba(0,0,0,.30);overflow:visible;text-align:left}
      .sac-panel.sac-listas-panel{width:min(720px,calc(100vw - 16px));min-width:min(720px,calc(100vw - 16px));max-width:min(720px,calc(100vw - 16px));max-height:calc(100vh - 16px)}
      ...
      @media (max-width:760px){.sac-panel{left:8px;right:8px;width:auto;min-width:0;max-width:none}.sac-grid.three,.sac-grid,.sac-field-grid{grid-template-columns:1fr}.sac-history-body{grid-template-columns:1fr}.sac-pid-grid{grid-template-columns:1fr}}
    `;
  ...
  function renderPanel({ id, stage, flow = "banking", subtitle = "", body = "", footer = "", onEnter, onSignature }) {
    ...
    const panel = document.createElement("div");
    panel.id = id;
    panel.className = `sac-panel sac-${getTheme()}${stage === "LISTAS" ? " sac-listas-panel" : ""}`;
  ...
  function merchantHistoryAlert(value) { return dropdownAlert(value, MERCHANT_HISTORY_OPTIONS); }
  function purchasePatternAlert(value) { return dropdownAlert(value, PURCHASE_PATTERN_OPTIONS); }
  ...
  function collectConsoleData(falcon) {
    ...
    const treatmentKind = /global|dock\s*one|dock\s*on|dockone|dockon/i.test(`${treatment} ${bodyText()}`) ? "global" : "brasil";
  ...
  function consoleGrid(data, options = {}) {
    ...
    if (data.flow === "card") {
      ...
      return `<div class="sac-grid three">${kv("CPF/CNPJ", data.cpfCnpj)}${kv("Emissor", data.issuer)}${kv("Tratativa", data.treatmentLabel || data.treatment)}${kv("ID cartão", data.cardId)}${kv("Cartão", data.cardNumber)}${kv("Status cartão", data.cardStatus)}${kv("Cadastro", data.registrationDate, alertIf(isRecentRegistration(data.registrationDate)))}</div>`;
    }
    return `<div class="sac-grid three">${kv("CPF/CNPJ", data.cpfCnpj)}${kv("Emissor", data.issuer)}${kv("Tratativa", data.treatmentLabel || data.treatment)}${kv("Conta", data.account)}${includeAccountStatus ? kv("Status conta", data.accountStatus, accountStatusAlert(data.accountStatus)) : ""}${kv("Cadastro", data.registrationDate, alertIf(isRecentRegistration(data.registrationDate)))}</div>`;
  }
  function cardFields(data) {
    ...
    return call
      + field("sac-merchant-history", "Histórico no estabelecimento", MERCHANT_HISTORY_OPTIONS, data.fields.merchantHistory, { allowEmpty: true })
      + field("sac-purchase-pattern", "Padrão de compra", PURCHASE_PATTERN_OPTIONS, data.fields.purchasePattern, { allowEmpty: true });
  }
  ...
  function consoleDropdownGrid(data) {
    if (data.flow === "card") {
      ...
      return `<div class="sac-grid">${kv("Chamada", data.fields.callMode || "sem chamada")}${callResult}${kv("Histórico no estabelecimento", data.fields.merchantHistory, merchantHistoryAlert(data.fields.merchantHistory))}${kv("Padrão de compra", data.fields.purchasePattern, purchasePatternAlert(data.fields.purchasePattern))}</div>`;
    }
