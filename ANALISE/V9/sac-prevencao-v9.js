(async function SacPrevencaoV9() {
  ...
  const memory = window.SACMemoryV9;
  const tabulatorEngine = window.SACTabulatorV9;
  if (!memory || !tabulatorEngine) throw new Error("Motores da V9 não foram carregados.");
  ...
  const SETTINGS_WINDOW_NAME_KEY = "__SAC_PREVENCAO_V9_SETTINGS__=";
  ...
  function ensureStyles() {
    if (byId("sac-style-V9")) return;
    const style = document.createElement("style");
    style.id = "sac-style-V9";
    style.textContent = `
  ...
  window.__SAC_PREVENCAO_V9_KEYS = (event) => {
  ...
  document.addEventListener("keydown", window.__SAC_PREVENCAO_V9_KEYS);
  ...
  function showFinalTabulation(data, decision, text, panel, status = "") {
    ...
    byId("sac-copy-final")?.addEventListener("click", async (event) => {
      ...
      if ((queue.length || history.length) && !clipboardEnvelopeReady) {
        showNotice("Tabulação copiada. LISTAS e Histórico foram mantidos na memória local da V9.", "warn", 11000);
      }
  ...
  async function readListQueue() {
    ...
    memory.lists.all().forEach((item) => {
      ...
      const identityKey = `${caseKey}:${accountKey}:${Boolean(item?.lists?.allowlist)}:${Boolean(item?.lists?.contencao)}`;
      const keyValue = caseKey || accountKey ? identityKey : item?.id;
