...
const context = {
  ...
  window: {
    ...
    SACCounterpartyV12: { validateCnpj: (value) => value === cnpj }
  }
};
...
(async () => {
  ...
  context.window.SACCorporateV12.useProvider({ load: async () => { throw new Error("REGISTRY_UNAVAILABLE"); } });
  const fallback = await context.window.SACCorporateV12.lookup(cnpj, { forceRefresh: true });
  ...
  console.log("OK - consulta cadastral automática V12 validada");
})().catch((error) => {
  console.error(error);
  process.exitCode = 1;
});
