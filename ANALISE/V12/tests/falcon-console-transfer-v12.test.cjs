...
assert.match(runtimeSource, /storeMediaRequest\(\{ falcon: data, visualFlow: data\.visualFlow, flow: data\.flow \}\);/);
assert.doesNotMatch(runtimeSource, /if \(getInvestigationMode\(\)\) storeMediaRequest/);
...
(async () => {
  ...
  const packageContext = { Date, BUILD_FAMILY: "12", BUILD_VERSION: "12.6", PACKAGE_SCHEMA: 1, PACKAGE_TTL_MS: 12 * 60 * 60 * 1000 };
  ...
  assert.equal(packageContext.isCurrentPackage({ ...currentFalcon, buildVersion: "12.5" }, "SAC_FALCON"), true, "subversões V12 compatíveis não podem perder o caso durante a troca de página");
