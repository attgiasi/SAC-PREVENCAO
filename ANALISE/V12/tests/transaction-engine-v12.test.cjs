...
const source = fs.readFileSync(path.join(__dirname, "..", "sac-transaction-v12.js"), "utf8");
vm.runInContext(source, context, { filename: "sac-transaction-v12.js" });

const engine = context.window.SACTransactionV12;
assert.ok(engine);
...
assert.equal(falconSummary.totalAmount, 0, "transações barradas pela regra não compõem a movimentação efetiva");
assert.equal(falconSummary.attemptedAmount, 2000);
assert.equal(falconSummary.effectiveTransactionCount, 0);
assert.equal(falconSummary.blockedAttemptCount, 2);
assert.equal(falconSummary.uniqueCounterpartyCount, 0);
assert.equal(falconSummary.p2pDetected, false, "o nome da regra não pode simular uma transação P2P");
assert.equal(falconSummary.counterparties.length, 0);
assert.equal(falconSummary.validDateCount, 2);
assert.equal(falconSummary.periodEnd - falconSummary.periodStart, 8 * 60 * 1000);
assert.equal(falconSummary.velocity1m, 0);
assert.equal(falconSummary.velocity5m, 0);
...
const escalatingTransactions = engine.transactionMetrics([
  { timestamp: new Date(2026, 6, 16, 10, 0, 0).getTime(), amount: 100, direction: "DEBIT" },
  { timestamp: new Date(2026, 6, 16, 10, 5, 0).getTime(), amount: 1200, direction: "DEBIT" }
]);
assert.equal(escalatingTransactions.lowToHighEscalations, 1, "teste baixo seguido de valor alto deve gerar atenção");
const decreasingTransactions = engine.transactionMetrics([
  { timestamp: new Date(2026, 6, 16, 10, 0, 0).getTime(), amount: 1200, direction: "DEBIT" },
  { timestamp: new Date(2026, 6, 16, 10, 5, 0).getTime(), amount: 100, direction: "DEBIT" }
]);
assert.equal(decreasingTransactions.lowToHighEscalations, 0, "valor alto seguido de teste baixo não deve ser interpretado como escalada");
...
(async () => {
  ...
  console.log("OK - motor transacional V12 validado");
})().catch((error) => {
  console.error(error);
  process.exitCode = 1;
});
