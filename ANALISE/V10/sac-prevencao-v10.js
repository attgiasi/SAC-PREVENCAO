(async function SacPrevencaoV10() {
  "use strict";
  ...
  const LIST_ENGINE_WINDOW_KEY = "__SAC_PREVENCAO_V10_LIST_ENGINE__=";
  const LIST_ENGINE_GLOBAL = "__SAC_PREVENCAO_V10_LIST_ENGINE__";
  ...
  const memory = window.SACMemoryV10;
  const tabulatorEngine = window.SACTabulatorV10;
  if (!memory || !tabulatorEngine) throw new Error("Motores da V10 não foram carregados.");
  ...
  const SETTINGS_WINDOW_NAME_KEY = "__SAC_PREVENCAO_V10_SETTINGS__=";
  ...
  function writeWindowNameBlock(marker, value) {
    try {
      const name = String(window.name || "");
      const cleanName = name.includes(marker)
        ? name.slice(0, name.indexOf(marker)).replace(/\n+$/, "")
        : name;
  ...
  function clearPreviousRuntime() {
    ...
    Object.keys(window)
      ...
      .forEach((handlerName) => {
        ...
        if (handlerName !== "__SAC_PREVENCAO_V10_KEYS") {
  ...
  function ensureStyles() {
    if (byId("sac-style-V10")) return;
    const style = document.createElement("style");
    style.id = "sac-style-V10";
    style.textContent = `
      ...
      .sac-panel.sac-listas-panel{--sac-panel-width:min(720px,calc(100vw - 16px));left:8px!important;right:auto!important;top:8px!important;max-height:calc(100vh - 16px)}
    ...
    panel.innerHTML = `
      <div class="sac-head">
        ...
        <div class="sac-actions">
          ...
          <button class="sac-icon" data-action="history" aria-label="Histórico">◷</button>
  ...
  window.__SAC_PREVENCAO_V10_KEYS = (event) => {
  ...
  document.addEventListener("keydown", window.__SAC_PREVENCAO_V10_KEYS);
  ...
  function showFinalTabulation(data, decision, text, panel, status = "") {
    ...
    byId("sac-copy-final")?.addEventListener("click", async (event) => {
      ...
      if ((queue.length || history.length) && !clipboardEnvelopeReady) {
        showNotice("Tabulação copiada. LISTAS e Histórico foram mantidos na memória local da V10.", "warn", 11000);
      }
      ...
      if (!initialIssuerId) {
        issuerIdForName(data.issuer).then((resolvedIssuerId) => {
          ...
          const refreshed = readImmediateListMirror().map((entry) => (
            sameListIdentity(entry, data) ? { ...entry, issuerId: resolvedIssuerId, updatedAt: Date.now() } : entry
          ));
          writeListQueue(refreshed);
        }).catch(() => {});
      }
      return next;
    });
