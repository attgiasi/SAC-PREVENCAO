...
assert.equal(release.build, "11.33");
assert.match(bookmarklet, new RegExp(`@${release.commit}/ANALISE/V11/loader-v11\\.js\\?v=11\\.33\\.0`));
...
(async () => {
  ...
  assert.ok(viaApi.loaded.every((url) => url.includes("v=11.33.0-")));
