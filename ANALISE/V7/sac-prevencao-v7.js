(async function SacPrevencaoV7() {
  "use strict";

  const APP = "sac_prevencao_v7_20260622";
  const BUILD = "ANALISE/V7";
  ...
  const memory = window.SACMemoryV7;
  const tabulatorEngine = window.SACTabulatorV7;
  if (!memory || !tabulatorEngine) throw new Error("Motores da V7 não foram carregados.");
  ...
  function clearPreviousRuntime() {
    all("[id^='sac-style'],.sac-panel,.sac-history-panel,.sac-choice-popover,#sac-notices").forEach((node) => node.remove());
  ...
  function ensureStyles() {
    if (byId("sac-style-v7")) return;
    const style = document.createElement("style");
    style.id = "sac-style-v7";
    style.textContent = `
      ...
      .sac-field-grid{display:grid;grid-template-columns:1fr 1fr;gap:4px}.sac-call-grid{grid-column:1/-1;display:grid;grid-template-columns:1fr 1fr;gap:4px}.sac-call-box{display:grid;grid-template-columns:1fr 1fr;gap:4px;border:1px solid var(--sac-border);border-radius:6px;background:var(--sac-card);padding:5px}.sac-call-box[aria-disabled="true"]{opacity:.52}.sac-call-box strong{grid-column:1/-1;color:var(--sac-muted);font-size:calc(8.4px * var(--sac-font-scale));text-transform:uppercase}.sac-call-box label,.sac-flag{display:flex;align-items:center;gap:5px;font-size:calc(9.8px * var(--sac-font-scale));font-weight:900;color:var(--sac-text)}.sac-flag{min-height:34px;border:1px solid var(--sac-border);border-radius:6px;background:var(--sac-card);padding:7px 8px;cursor:pointer}.sac-flag:hover{border-color:#38bdf8;background:#10263a;box-shadow:0 0 0 2px rgba(56,189,248,.12)}.sac-light .sac-flag:hover{background:#eef7ff;color:#172033}.sac-field{display:grid;gap:2px;min-width:0;border:1px solid var(--sac-border);border-radius:6px;background:var(--sac-card);padding:4px;transition:border-color .15s,box-shadow .15s,background .15s}.sac-field span{display:block;font-size:calc(8.4px * var(--sac-font-scale));font-weight:900;color:var(--sac-muted);line-height:1.05}.sac-field select,.sac-field input{width:100%;height:28px;border:1px solid var(--sac-border);border-radius:5px;background:var(--sac-input);color:var(--sac-text);font-size:calc(10.6px * var(--sac-font-scale));font-weight:800;padding:3px}.sac-field select:hover,.sac-field select:focus,.sac-field input:hover,.sac-field input:focus{border-color:#38bdf8;background:#10263a;color:#edf3fb;outline:none;box-shadow:0 0 0 2px rgba(56,189,248,.16)}.sac-light .sac-field select:hover,.sac-light .sac-field select:focus,.sac-light .sac-field input:hover,.sac-light .sac-field input:focus{background:#eef7ff;color:#172033}.sac-other-input[hidden]{display:none!important}
  ...
  window.__SAC_PREVENCAO_V7_KEYS = (event) => {
  ...
  document.addEventListener("keydown", window.__SAC_PREVENCAO_V7_KEYS);
  ...
  async function renderTabulator(existingData = null) {
    ...
    const body = section("Dados do Falcon", falconGrid(data.falcon, { enhancePix: true }), "análise")
      ...
      + section("JIRA", `<label class="sac-flag"><input type="checkbox" id="sac-jira-flag" ${data.jiraActive ? "checked" : ""}><span>Aplicar chamada RECEPTIVO com COM SUCESSO</span></label>`, "opcional")
  ...
  function fillObservationText(text) {
    ...
    targets.forEach((target) => {
      const element = targetElement(target);
      if (!element) return;
      ok = setNativeValue(element, text) || ok;
    });
    return ok;
  }
  async function applyPrimaryTabulatorFields(data, isActive = () => true) {
    ...
    const applyInput = (targets, value, label, tries = 6) =>
      forceFillAny(targets, value, tries, 25, isActive).then((ok) => {
    ...
    tasks.push((async () => {
      ...
      const typeApplied = await selectDropdown("ddl_tipoDoc", docKind, 24, isActive)
      ...
      return applyInput(targets, doc, docKind, 8);
    })());

    const callValues = tabulatorCallValues(data);
    const queue = queueFor(data);
    tasks.push((async () => {
      if (byId("ddl_tabulador") && !await selectDropdown("ddl_tabulador", "Falcon", 18, isActive)) addPending("Tabulador Falcon");
    })());
    tasks.push(selectIssuerDropdown(data.issuer, data.issuerId, isActive).then((ok) => {
      if (!ok) addPending("Emissor");
      return ok;
    }));
    tasks.push(selectDropdown("ddl_TipoChamada", callValues.type, 20, isActive).then((ok) => {
      if (!ok) addPending("Tipo de chamada");
      return ok;
    }));
    tasks.push(selectDropdown("ddl_ChamadaAtiva", callValues.result, 20, isActive).then((ok) => {
      if (!ok) addPending("Status chamada");
      return ok;
    }));
    if (queue) {
      tasks.push((async () => {
        let ok = await selectDropdown("ddl_Fila", queue, 40, isActive);
        if (!ok) ok = await selectDropdown("ddl_Fila", queue, 25, isActive);
  ...
  async function markListDone(id, listType) {
    ...
    const persisted = await writeListQueue(list);
    if (!persisted) memory.lists.replace(original);
    return persisted;
  }
