...
const context = {
  window: {
    ...
    SACCounterpartyV12: { validateCnpj: (value) => /^[A-Z0-9]{12}[0-9]{2}$/.test(value) }
  },
  URL,
  fetch: async () => { throw new Error("offline"); }
};
vm.createContext(context);
const source = fs.readFileSync(path.join(__dirname, "..", "sac-corporate-v12.js"), "utf8");
vm.runInContext(source, context, { filename: "sac-corporate-v12.js" });

const engine = context.window.SACCorporateV12;
...
console.log("OK - motor cadastral Receita V12 validado");
