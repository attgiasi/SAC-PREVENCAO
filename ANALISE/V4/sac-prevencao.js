(async function SacPrevencaoV4() {
  "use strict";

  const APP = "sac_prevencao_v4_20260618";
  const BUILD = "ANALISE/V4";
  ...
  const FRIENDLY_MESSAGES = {
    FALCON: [
      "Por favor, confira os dados coletados. Obrigado, vamos deixar essa análise redonda.",
      "Dados prontos para revisão. Respira tranquilo: eu te ajudo a seguir com segurança.",
      "Conferência do Falcon montada. Obrigado pela atenção aos detalhes."
    ],
    CONSOLE: [
      "Por favor, revise os dados do Console e escolha os dropdowns. Estamos quase lá.",
      "Dados recebidos e organizados. Obrigado, sua análise fica mais rápida daqui pra frente.",
      "Console pronto para conferência. Boa análise, seguimos juntos."
    ],
    TABULADOR: [
      "Por favor, avalie os grids e escolha a decisão. Depois eu aplico e preparo a tabulação.",
      "Tudo organizado para decisão. Obrigado pela revisão final.",
      "Tabulador preparado. Escolha com calma; eu cuido do preenchimento."
    ],
    LISTAS: [
      "Casos pendentes ficam guardados até você inserir ou remover.",
      "Escolha a aba e insira quando estiver na página correta. Obrigado.",
      "LISTAS pronta para aplicação dos casos pendentes."
    ]
  };
  ...
  const purgeLegacyState = () => {
    const legacyPrefixes = [
      "sac_prevencao_v1",
      "sac_prevencao_v2",
      "sac_prevencao_v3_20260617",
      "sac_prevencao_v3_2_20260617",
      "sac_prevencao_v4_old"
    ];
    try {
      const remove = [];
  ...
  if (storageGet("activeBuild") !== BUILD_VERSION) {
    ["lastFalcon", "lastConsole", "history", "listas", "allowlist"].forEach(storageRemove);
  ...
  const getRuleHelp = () => storageGet("ruleHelp") !== "off";
  const setRuleHelp = (enabled) => storageSet("ruleHelp", enabled ? "on" : "off");
  ...
  function nextFriendlyMessage(stage) {
    const keyName = `friendlyIndex:${stage}`;
    const list = FRIENDLY_MESSAGES[stage] || FRIENDLY_MESSAGES.FALCON;
    const index = (Number(storageGet(keyName)) || 0) % list.length;
    storageSet(keyName, String(index + 1));
    return list[index];
  }
  ...
  function ensureStyles() {
    ...
    style.id = "sac-style";
    style.textContent = `
      .sac-panel{--sac-font-scale:1;--sac-primary:#00856f;--sac-window-width:420px;position:fixed;top:8px;right:8px;z-index:2147483647;width:min(var(--sac-window-width),calc(100vw - 16px));border:1px solid var(--sac-border);border-top:3px solid var(--sac-primary);border-radius:8px;background:var(--sac-bg);color:var(--sac-text);font-family:Inter,Segoe UI,Arial,sans-serif;box-shadow:0 18px 44px rgba(0,0,0,.30);overflow:visible;text-align:left}
      ...
      .sac-choice-popover{position:fixed;right:14px;top:72px;z-index:2147483647;width:min(354px,calc(100vw - 28px));display:grid;gap:7px;padding:0 10px 10px;border:1px solid var(--sac-border);border-top:3px solid var(--sac-primary);border-radius:8px;background:var(--sac-bg);color:var(--sac-text);box-shadow:0 18px 44px rgba(0,0,0,.32);font-family:Inter,Segoe UI,Arial,sans-serif}.sac-choice-head{margin:0 -10px;padding:8px 10px;background:var(--sac-primary);color:#fff;display:flex;justify-content:space-between;align-items:center;border-radius:6px 6px 0 0}.sac-choice-head strong{font-size:12px}.sac-choice-head button{width:24px;height:24px;padding:0;border-color:rgba(255,255,255,.4);background:rgba(127,29,29,.32)}.sac-choice-grid{display:grid;grid-template-columns:1fr 1fr;gap:5px}.sac-choice-popover label{display:flex;gap:6px;align-items:flex-start;min-height:38px;border:1px solid var(--sac-border);border-radius:6px;background:var(--sac-card);padding:6px;font-size:10.5px;font-weight:850;line-height:1.12;cursor:pointer;overflow-wrap:anywhere}.sac-choice-popover label:hover{border-color:#38bdf8;background:#10263a;color:#edf3fb}.sac-light.sac-choice-popover label:hover{background:#eef7ff;color:#172033}.sac-choice-popover input{margin-top:1px}.sac-choice-extra{display:grid!important;gap:4px;min-height:0!important}.sac-choice-extra input{width:100%;height:30px;border:1px solid var(--sac-border);border-radius:6px;background:var(--sac-input);color:var(--sac-text);padding:5px;font-weight:850}.sac-choice-popover .sac-choice-actions{display:grid;grid-template-columns:1fr 1fr;gap:6px}.sac-choice-popover button{border:1px solid var(--sac-border);border-radius:6px;background:var(--sac-primary);color:#fff;padding:8px;font-weight:950;cursor:pointer}.sac-choice-popover button.secondary{background:transparent;color:var(--sac-text)}
  ...
  function renderPanel({ id, stage, flow = "banking", subtitle = "", body = "", footer = "", onEnter, onSignature }) {
    ...
    const friendlySubtitle = subtitle ? `${subtitle}. ${nextFriendlyMessage(stage)}` : nextFriendlyMessage(stage);
    ...
    const flowColorSwatches = (flowName) => FLOW_COLOR_OPTIONS
      .map(([color, label]) => `<button class="sac-color-swatch ${getFlowTone(flowName) === color ? "active" : ""}" style="--swatch:${color}" data-flow-color-choice="${escapeHtml(flowName)}" data-color="${color}" title="${escapeHtml(label)}" aria-label="${escapeHtml(label)}"></button>`)
      .join("");
    const colorConfig = `
    ...
    panel.innerHTML = `
      <div class="sac-head">
        <div>
          ...
          <div class="sac-subtitle">${escapeHtml(friendlySubtitle || BUILD)}</div>
        </div>
        ...
        <div class="sac-config" hidden>
          ...
          <button data-action="rule-help">${getRuleHelp() ? "ⓘ Ajuda da regra ligada" : "ⓘ Ajuda da regra desligada"}</button>
          ${signatureConfig}
    ...
    panel.querySelector("[data-action='rule-help']")?.addEventListener("click", (event) => {
      const next = !getRuleHelp();
      setRuleHelp(next);
      event.currentTarget.textContent = next ? "ⓘ Ajuda da regra ligada" : "ⓘ Ajuda da regra desligada";
      showNotice(next ? "Ajuda da regra ligada." : "Ajuda da regra desligada.", "info");
      reload();
    });
  ...
  function ruleHelpText(rule) {
    const text = normalize(rule);
    if (text.includes("HOLD")) return "HOLD: tratar com atenção. Não aplicar SPD15 e não enviar para LISTAS. Não fraude libera/aprova HOLD; fraude declina/aplica HOLD. Se houver múltiplas HOLDs, tratar todas.";
    if (text.includes("CONTENCAO") || text.includes("CONTENSAO")) return "CONTENÇÃO: em decisão NÃO FRAUDE, enviar para LISTAS em Allowlist e também em Contenção usando CPF/CNPJ sem pontuação.";
    if (text.includes("DENYLIST_EC")) return "DENYLIST_EC: risco no estabelecimento. Validar com cliente antes da decisão.";
    if (text.includes("DENYLIST_CCS")) return "DENYLIST_CCS: validar se o cliente reconhece a transação que originou o bloqueio do estabelecimento.";
    if (text.includes("DENYLIST")) return "DENYLIST: análise completa antes da decisão. Não decidir por apenas um indício.";
    if (text.includes("CAPITAL") || text.includes("AUTO")) return "Capital de giro/auto fraude: avaliar contato, padrão de compra e orientação específica do emissor antes de bloquear.";
    return "Fluxo de lista: BANKING apenas. Falcon > Console > Tabulador > decisão NÃO FRAUDE > guarda em LISTAS até inserir no Falcon Allowlist.";
  }
  function kv(label, value, cls = "") {
    const missing = isMissing(value);
    const ruleInfo = normalize(label) === "REGRA" && getRuleHelp()
      ? `<span class="sac-rule-info" title="${escapeHtml(ruleHelpText(value))}">!</span>`
      : "";
    return `<div class="sac-kv ${missing ? "sac-missing" : ""} ${cls}"><div class="sac-kv-label">${escapeHtml(label)}</div>${ruleInfo}<div class="sac-kv-value">${escapeHtml(clean(value))}</div></div>`;
  }
  function kvOptional(label, value, cls = "") {
    const ruleInfo = normalize(label) === "REGRA" && getRuleHelp()
      ? `<span class="sac-rule-info" title="${escapeHtml(ruleHelpText(value))}">!</span>`
      : "";
    return `<div class="sac-kv ${cls}"><div class="sac-kv-label">${escapeHtml(label)}</div>${ruleInfo}<div class="sac-kv-value">${escapeHtml(clean(value, ""))}</div></div>`;
  }
  ...
  function enableDrag(panel, handleSelector) {
    ...
    const freezePanelPosition = () => {
      ...
      panel.style.minWidth = `${startWidth}px`;
      panel.style.maxWidth = `${startWidth}px`;
      panel.style.right = "auto";
  ...
  function selectHoldRowsCheckboxes() {
    ...
    all("tr,[role='row']").forEach((row) => {
      if (!normalize(textOf(row)).includes("HOLD")) return;
      const checkbox = all("input[type='checkbox']", row)[0];
      if (!checkbox) return;
      found += 1;
      if (!checkbox.checked) {
        try { checkbox.click(); } catch (_err) {}
  ...
  function defaultConsoleFields(flow, isGlobal) {
    if (flow === "card") return isGlobal
      ? { callMode: "sem chamada", callResult: "", merchantHistory: "ausência de dados", purchasePattern: "ausência de dados" }
      : { callMode: "sem chamada", callResult: "", merchantHistory: "", purchasePattern: "" };
    return {
      badMedia: "não",
      personStatus: isGlobal ? "" : "normal",
      emailPhoneAddress: "de acordo",
      spdHistory: isGlobal ? "" : "não",
  ...
  async function renderConsole() {
    ...
    if (data.flow === "card" && !data.isGlobal && data.cardMatched === false) {
      const panel = renderPanel({
    ...
    all("input[name='sac-call-mode']", panel).forEach((input) => input.addEventListener("change", syncCardCall));
    syncCardCall();
    enableManualGridEditing(panel, data);
  ...
  function consoleGrid(data, options = {}) {
    ...
    if (data.flow === "card") {
      if (!data.isGlobal && data.cardMatched === false) {
  ...
  async function readConsoleFields(data) {
    ...
    const missingManual = [];
    const personStatus = data.isGlobal ? "" : resolveOtherOption(byId("sac-person-status")?.value || "normal", "Status Pessoa (SPD)", missingManual, document.querySelector("[data-other-for='sac-person-status']")?.value);
    const spdHistory = data.isGlobal ? "" : resolveOtherOption(byId("sac-spd-history")?.value || "não", "Histórico SPD", missingManual, document.querySelector("[data-other-for='sac-spd-history']")?.value);
    return {
  ...
  async function applyDecisionAndShowFinal(data, decision, panel) {
    ...
    if (data.flow === "card" && analysisMissing.length) {
      showNotice(`Para continuar no fluxo CARTÃO, selecione: ${analysisMissing.join(", ")}.`, "error", 12000);
      return;
    }
    const decisionButtons = all("[data-decision]", panel);
    ...
    showFinalTabulation(data, decision, text, panel, "Aplicando campos no Tabulador. A tela já está pronta para conferência.");
    await copyText(text);
    updateFinalStatus(panel, "Tabulação copiada. Estou aplicando os campos no Tabulador.", "ok");
    await wait(0);
    await forceFillAny(observationTargets, text, 8, 60);
    const missing = requiredWorkflow(data);
    if (missing.length) {
      if (getSafeMode()) {
        showNotice(`Ainda faltam dados para decidir: ${missing.join(", ")}. Confira os grids em laranja.`, "error");
        updateFinalStatus(panel, `Faltam dados obrigatórios: ${missing.join(", ")}. Use Mudar decisão para corrigir.`, "error");
        return;
      }
      showNotice(`Atenção: faltam dados (${missing.join(", ")}), mas o modo seguro está desligado.`, "warn");
    }
    const applied = await applyTabulator(data, decision, text);
    if (!applied.ok) {
      if (getSafeMode()) {
        showNotice(`Revise o Tabulador: ${applied.pending.join(", ")}. A tabulação já foi colocada em Observações.`, "warn-pulse", 15000);
        updateFinalStatus(panel, `Revise no Tabulador: ${applied.pending.join(", ")}. A tabulação ficou disponível abaixo.`, "warn");
    ...
    const observationApplied = await forceFillAny(observationTargets, text, 8, 60);
    ...
    updateFinalStatus(panel, applied.ok ? "Campos aplicados e tabulação copiada." : "Tabulação copiada. Confira os campos sinalizados no Tabulador.", applied.ok ? "ok" : "warn");
  }
  function showFinalTabulation(data, decision, text, panel, status = "") {
    panel.querySelector(".sac-body").innerHTML =
      ...
      + `<div class="sac-final-actions"><button class="sac-main" id="sac-copy-final">Copiar</button><button class="sac-secondary" id="sac-change-decision">Mudar decisão</button></div>`;
  ...
  async function selectDependentDropdown(parentId, parentWanted, childId, childWanted, tries = 60) {
  ...
  async function applyTabulator(data, decision, tabulationText = "") {
    const pending = [];
    const f = data.falcon || {};
    ...
    if (byId("ddl_tabulador") && !await selectDropdown("ddl_tabulador", "Falcon", 18)) addPending("Tabulador Falcon");
    ...
    if (!await selectDropdown("ddl_TipoChamada", callValues.type, 18)) addPending("Tipo de chamada");
    if (!await selectDropdown("ddl_ChamadaAtiva", callValues.result, 18)) addPending("Status chamada");
    const queue = queueFor(data);
    if (queue && !await selectDropdown("ddl_Fila", queue, 28)) addPending("Fila");
    if (!await selectDropdown("ddl_status", decision, 35)) addPending("Status");
    const reason = reasonForDecision(data.flow, decision);
    if (reason) {
      if (!await selectDependentDropdown("ddl_status", decision, "ddl_motivostatus", reason, 60)) addPending("Motivo status");
    }
    primeTabulatorFields(data, decision, tabulationText);
  ...
  function readListQueue() {
    const primary = readJson("listasMemory") || [];
    const legacy = readJson("listas") || readJson("allowlist") || [];
