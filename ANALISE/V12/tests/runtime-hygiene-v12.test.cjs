...
assert.doesNotMatch(runtime, /hydrateListClipboardFast|markListBatchDone|hydrateClipboard:/);
assert.match(runtime, /let issuerDirectoryPromise = null;/);
assert.match(runtime, /if \(issuerDirectoryPromise\) return issuerDirectoryPromise;/);
...
assert.match(runtime, /function tabulatorTextFieldPlan\(data, tabulationText = ""\)/);
assert.match(runtime, /async function confirmTabulatorTextFields\(data, tabulationText/);
assert.match(runtime, /const textAudit = await confirmTabulatorTextFields\(data, tabulationText, isActive\);/);
assert.match(runtime, /"Data de entrada", "Hora de entrada"[\s\S]+?"Número do caso"[\s\S]+?"Observações"/);
assert.match(runtime, /all\("\[data-history-id\]", listEl\)\.forEach\(\(entry\) => entry\.classList\.toggle\("active", entry === button\)\);/);
assert.match(runtime, /\.sac-history-panel\{[^}]*box-sizing:border-box!important/);
assert.match(runtime, /Object\.getOwnPropertyNames\(window\)[\s\S]+?__SAC_PREVENCAO_V\\d\+_RUNTIME__/);
assert.match(runtime, /\.sac-side-panel,\.sac-pid-panel,#sac-notices/, "a inicialização deve remover um PID órfão");
...
for (const setting of ["theme", "safeMode", "invisibleMode", "investigationMode", "helpMode", "fontScale", "signatureName", "signatureSector"]) {
...
assert.match(runtime, /await memory\.hydrateFromClipboard\(\{ timeoutMs: 1100 \}\)/);
...
assert.match(memory, /function mergeCurrentSettings\(\)/);
assert.match(memory, /function preserveProgrammaticText\(text, nativeWriteText\)/);
assert.match(memory, /const settings = \{[\s\S]+?get\(name\) \{\s*mergeCurrentSettings\(\);/);
assert.match(runtime, /function installProgrammaticCopyPreserver\(\)/);
...
assert.match(preview, /preferred=rect\.left-width-4/);
assert.match(preview, /invisible:false,investigation:false,help:false/);
assert.match(preview, />Modo invisível</);
