  ...
  function openContextHelp(kind, value, ownerPanel) {
    ...
    const groups = entries.map((entry) => `
      <div class="sac-side-group">
        ...
        ${entry.items.map((item) => `<div class="sac-side-card"><span>${escapeHtml(item)}</span></div>`).join("")}
      </div>
    `).join("");
    panel.innerHTML = `<div class="sac-side-head"><span>${kind === "issuer" ? "Ajuda do emissor" : "Ajuda da regra"}</span></div><div class="sac-side-body"><div class="sac-side-meta">${escapeHtml(value || "N/A")}</div>${groups}</div>`;
    document.body.appendChild(panel);
