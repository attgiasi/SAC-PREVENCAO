...
const memorySource = fs.readFileSync(path.join(root, "sac-memory-v11.js"), "utf8");
const runtimeSource = fs.readFileSync(path.join(root, "sac-prevencao-v11.js"), "utf8");
...
function memoryContext({ clipboardHtml = "", sessionInitial = {} } = {}) {
  ...
  vm.runInContext(memorySource, context, { filename: "sac-memory-v11.js" });
  return context;
}
...
const clipboardHtml = `<div>SAC_FALCON</div><!--SAC_PREVENCAO_MEMORY_V11:${encodeURIComponent(JSON.stringify(portable))}-->`;
const destination = memoryContext({
  clipboardHtml,
  sessionInitial: {
    "sac_prevencao_V11:transport": JSON.stringify({ falcon: staleFalcon })
  }
});

(async () => {
  const beforeRead = destination.window.SACMemoryV11.snapshot().savedAt;
  destination.window.SACMemoryV11.mergeCurrentMirrors();
  assert.equal(
    destination.window.SACMemoryV11.snapshot().savedAt,
    beforeRead,
    "uma leitura da memória não pode se apresentar como gravação mais recente"
  );

  await destination.window.SACMemoryV11.hydrateFromClipboard({ timeoutMs: 100 });
  const received = destination.window.SACMemoryV11.transport.get("falcon");
  ...
  const packageContext = { Date, BUILD_FAMILY: "11", BUILD_VERSION: "11.28", PACKAGE_SCHEMA: 1, PACKAGE_TTL_MS: 12 * 60 * 60 * 1000 };
  ...
  assert.equal(packageContext.isCurrentPackage({ ...currentFalcon, buildVersion: "11.23" }, "SAC_FALCON"), true, "subversões V11 compatíveis não podem perder o caso durante a troca de página");
